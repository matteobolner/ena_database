...
df = df.drop(
    columns=[
        "country_lookup",
        "state_by_coordinates",
        "country_by_coordinates",
        "sex_ena_db",
        "sex_bs_db",
        "sex",
        "ploidy",
...
df["geographic_location_manually_curated"] = df[
    "geographic_location_manually_curated"
].fillna("Unknown")
...
samplesdf = pd.DataFrame(
    index=df["accession"].unique(),
    columns=["species", "subspecies", "sex", "geographic_location", "pooled_samples"],
)
...
for name, group in df.groupby(by="accession"):
    ...
    templist.append(
        ",".join(group["apis_subspecies"].fillna("unknown").unique().tolist())
    )
    templist.append(
        ",".join(group["sex_manually_curated"].fillna("unknown").unique().tolist())
    )
    templist.append(
        ",".join(
            group["geographic_location_manually_curated"]
            .fillna("unknown")
            .unique()
            .tolist()
        )
    ...
    templist.append(
        ",".join(group["pooled_samples"].fillna("unknown").unique().tolist())
    )
...
plt.clf()
ax = sns.countplot(
    data=samplesdf_mellifera,
    x="subspecies",
    order=samplesdf_mellifera["subspecies"].value_counts().index,
    palette="muted",
)
for item in ax.get_xticklabels():
    item.set_rotation(90)
...
plt.clf()
ax = sns.countplot(
    data=samplesdf_non_mellifera,
    x="subspecies",
    order=samplesdf_non_mellifera["subspecies"].value_counts().index,
    palette="muted",
)
for item in ax.get_xticklabels():
    item.set_rotation(90)
...
ax = sns.countplot(
    data=samplesdf_non_mellifera,
    x="species",
    order=samplesdf_non_mellifera["species"].value_counts().index,
    palette="muted",
)
for item in ax.get_xticklabels():
    item.set_rotation(90)
...
ax = sns.countplot(
    data=samplesdf_mellifera,
    x="sex",
    order=samplesdf_mellifera["sex"].value_counts().index,
    palette="muted",
)
ax.set_xlabel("Sex")
ax.bar_label(ax.containers[0])
...
ax = sns.countplot(
    data=samplesdf_non_mellifera,
    x="sex",
    order=samplesdf_non_mellifera["sex"].value_counts().index,
    palette="muted",
)
ax.set_xlabel("Sex")
ax.bar_label(ax.containers[0])
...
ax = sns.countplot(
    data=samplesdf,
    x="sex",
    order=samplesdf["sex"].value_counts().index,
    palette="muted",
)
ax.set_xlabel("Sex")
ax.bar_label(ax.containers[0])
...
country_code = pc.country_name_to_country_alpha2(
    "Switzerland", cn_name_format="default"
)
...
for country in countries:
    ...
    if len(a) > 1:
        ...
        country_codes.append(
            pc.country_name_to_country_alpha2(i, cn_name_format="default")
        )
    else:
        a = a[0]
        # print(a)
...
ax = sns.countplot(
    data=samplesdf,
    x="continent",
    order=samplesdf["continent"].value_counts().index,
    palette="muted",
)
...
ax = sns.countplot(
    data=samplesdf_mellifera,
    x="continent",
    order=samplesdf_mellifera["continent"].value_counts().index,
    palette="muted",
)
ax.set_xlabel("Continent")
ax.bar_label(ax.containers[0])
...
ax = sns.countplot(
    data=samplesdf_non_mellifera,
    x="continent",
    order=samplesdf_non_mellifera["continent"].value_counts().index,
    palette="muted",
)
ax.set_xlabel("Continent")
ax.bar_label(ax.containers[0])
...
ax = sns.countplot(
    data=samplesdf,
    x="pooled_samples",
    order=samplesdf["pooled_samples"].value_counts().index,
    palette="muted",
)
...
ax = sns.countplot(
    data=samplesdf_mellifera,
    x="pooled_samples",
    order=samplesdf_mellifera["pooled_samples"].value_counts().index,
    palette="muted",
)
ax.set_xlabel("Pooled samples")
ax.bar_label(ax.containers[0])
...
ax = sns.countplot(
    data=samplesdf_non_mellifera,
    x="pooled_samples",
    order=samplesdf_non_mellifera["pooled_samples"].value_counts().index,
    palette="muted",
)
ax.set_xlabel("Pooled samples")
ax.bar_label(ax.containers[0])
