...
class ena_db:
    ...
    def __init__(
        ...
        if db_path:
            self.ena_df=pd.read_table(db_path)
        elif create_db == True:
    ...
    def build_biosamples_df(self, df, thread_number=30):
        ids = df["sample_accession"].dropna().unique().tolist()
        ...
        biosamples_df["domain"] = biosamples_df["domain"].str.replace(
            "self.BiosampleImport", ""
        )
        biosamples_df.columns = biosamples_df.columns.str.replace(
            "characteristics.", ""
        )
    ...
    def clean_biosamples_df(self, df, thread_number=30):
        ...
        def clean_row(row):
            acceptables_list = [str, bool, float, int, np.int64, np.bool_]
            pbar.update(1)
            newrow = []
            for col in df.columns:
